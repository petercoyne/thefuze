<script>
	...
	let tracks = [
		...
		{
			name: 'Mystical Forces (Live in Salthill)',
			length: '',
			src: '/music/16-mystical-forces-salthill.mp3'
		},
		{
			name: 'Ive Heard Angels (Live in Maam Cross)',
			length: '',
			src: '/music/17-ive-heard-angels-maam-cross.mp3'
		},
		{ name: 'Invasion (Live in Tramore)', length: '', src: '/music/18-invasion-tramore.mp3' },
		{
			name: 'Look What Shes Done To Me (Live in Dublin)',
			length: '',
			src: '/music/19-look-what-shes-done-to-me-dublin.mp3'
		},
		{
			name: 'Shining Star (Live in Claremorris)',
			length: '',
			src: '/music/20-shining-star-claremorris.mp3'
		}
	];
...
>
			...
			<div class="hidden sm:block pl-6">
				...
				<p class="text-center">
					<a
						...
						class="text-center my-4">Buy CD</a
					>
				</p>
			</div>
	...
	<div
		class="sticky bottom-0 bg-black md:py-2"
		style="background-image: url('bg-gradient.png'); background-repeat: repeat-x;"
	>
		...
		<div class="flex items-center" transition:slide>
			...
			<div
				class="cursor-pointer text-s px-0 h-16 flex flex-col justify-center pr-4 md:pl-6"
				on:click={() => playing.set(1)}
			>
				<div class="text-xs uppercase text-gray-400">The Fuze</div>
				<div class="text-xs sm:text-sm">{currentTrackName}</div>
			</div>

			<div id="waveform" class="flex-grow overflow-hidden" transition:slide>
				<!-- Here be waveform -->
			</div>
